...
public class CFMLProxy {
	...
	public Object invokeBody(AMFCaster caster, ServletContext context, ServletConfig config, HttpServletRequest req,
			HttpServletResponse rsp, String serviceName, String serviceMethodName, List rawParams)
			throws ServletException, PageException, IOException {
		...
		try {
			...
			if (Log.isDebug()) {
				Log.getLogger("Lucee.Ext.Flex").debug("cfc: " + cfc);
			}
			...
			if (Log.isDebug()) {
				Log.getLogger("Lucee.Ext.Flex").debug("querystring: " + qs);
			}
			...
			if (Log.isDebug()) {
				Log.getLogger("Lucee.Ext.Flex").debug("raw parameters");
			...
			if (Log.isDebug()) {
				Log.getLogger("Lucee.Ext.Flex").debug("cast parameters");
			...
			if (source != null) {
				...
				if (Log.isDebug()) {
					Log.getLogger("Lucee.Ext.Flex").debug("page context");
	...
	private void writeBackResponse(PageContext pc, HttpServletResponse rsp) throws PageException {
		...
		while (it.hasNext()) {
			...
			if (value instanceof Long)
				rsp.addDateHeader(e.getKey(), ((Long) value).longValue());
			else if (value instanceof Integer)
				rsp.addDateHeader(e.getKey(), ((Integer) value).intValue());
			else
				rsp.addHeader(e.getKey(), caster.toString(e.getValue(), ""));
		}
	}
	...
	private PageContext createPageContext(CFMLFactory factory, String scriptName, String queryString, OutputStream os,
		...
		HttpServletRequest req = creator.createHttpServletRequest(new File(root.getAbsolutePath()), "localhost",
				scriptName, queryString, HttpUtil.cloneCookies(formerReq), headers, HttpUtil.cloneParameters(formerReq),
				HttpUtil.cloneAttributes(formerReq), null);

		// Response
		if (os == null)
			os = DevNullOutputStream.DEV_NULL_OUTPUT_STREAM;
	...
	private void print(PageContext pc, String str) {
		lucee.commons.io.log.Log log = pc.getConfig().getLog("application");
		log.info("flex", str);
	}
